...
const RegisterForm = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    if (response.success) {
      toast.success(response.message || "Success register user", {
        description : "Please login to continue",
      })
      router.push(`/login`)
    } else {
      toast.error(response.message || "Failed register user")
    }
  }
  ...
  return (
    <Card className="w-full max-w-sm shadow-2xl border-none bg-white/10 backdrop-blur-md">
      <CardHeader>
        <CardTitle className="text-white text-center text-2xl">Register</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          >
            {isSubmitting ? <LoaderCircleIcon className='animate-spin text-white' /> : 'Register'}
          </Button>
        </form>
        <div className="text-center mt-4 text-gray-100">
          <p className='text-sm'>Already have an account?{' '}
            <Link href={"/login"}>
              <span className='text-white font-semibold text-sm cursor-pointer'>Login</span>
            </Link>
          </p>
        </div>
      </CardContent>
